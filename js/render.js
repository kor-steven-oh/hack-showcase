...
const ACT_TXT={game:'게임하기',idea:'아이디어 남기기',info:'안내 보기'};
...
function drawSofa(d){
  ...
  ctx.save();ctx.translate(x,y);ctx.scale(1,0.5);ctx.fillStyle=P.charShadow;ctx.beginPath();ctx.ellipse(0,0,36,18,0,0,Math.PI*2);ctx.fill();ctx.restore();
...
function drawLongtable(d){ // 긴 테이블 — 아이소 축 정렬, 양쪽 착석용
  ...
  ctx.save();ctx.translate(x,y);ctx.scale(1,0.5);ctx.fillStyle=P.charShadow;ctx.beginPath();ctx.ellipse(0,0,44,16,0,0,Math.PI*2);ctx.fill();ctx.restore();
...
function drawCafebar(d){
  ...
  const o=isoO(x,y,1);
  ctx.save();ctx.translate(x,y);ctx.scale(1,0.5);ctx.fillStyle=P.charShadow;ctx.beginPath();ctx.ellipse(0,0,66,20,0,0,Math.PI*2);ctx.fill();ctx.restore();
  // 바리스타 (바 뒤 상주 판매원)
  const[px,py]=o(8,-13);
...
function drawVending(d){
  ...
  const dir=(d.sk||0.5)<0?-1:1,o=isoO(x,y,dir);
  ctx.save();ctx.translate(x,y);ctx.scale(1,0.5);ctx.fillStyle=P.charShadow;ctx.beginPath();ctx.ellipse(0,0,20,11,0,0,Math.PI*2);ctx.fill();ctx.restore();
...
function drawFoodcase(d){ // 음식 진열대 — 카운터 + 유리 진열부 + 페이스트리
  ...
  const dir=(d.sk||0.5)<0?-1:1,o=isoO(x,y,dir);
  ctx.save();ctx.translate(x,y);ctx.scale(1,0.5);ctx.fillStyle=P.charShadow;ctx.beginPath();ctx.ellipse(0,0,24,12,0,0,Math.PI*2);ctx.fill();ctx.restore();
...
function drawFridge(d){ // 음료 진열대(쿨러) — 선반 3단 + 병
  const x=d.sx,y=d.sy;if(cull(x,y-80))return;
  const dir=(d.sk||0.5)<0?-1:1,o=isoO(x,y,dir);
  ctx.save();ctx.translate(x,y);ctx.scale(1,0.5);ctx.fillStyle=P.charShadow;ctx.beginPath();ctx.ellipse(0,0,18,10,0,0,Math.PI*2);ctx.fill();ctx.restore();
...
function drawDesk(d,t){
  ...
  const o=isoO(x,y,1);
  ctx.save();ctx.translate(x,y);ctx.scale(1,0.5);ctx.fillStyle=P.charShadow;ctx.beginPath();ctx.ellipse(0,0,54,17,0,0,Math.PI*2);ctx.fill();ctx.restore();
  // 스태프 (데스크 뒤 = -v 쪽)
  const[px,py]=o(0,-13);
...
function drawArcade(d,t){
  ...
  ctx.save();ctx.translate(x,y);ctx.scale(1,0.5);ctx.fillStyle=P.charShadow;ctx.beginPath();ctx.ellipse(0,0,32,16,0,0,Math.PI*2);ctx.fill();ctx.restore();
...
function drawBench(d){
  ...
  ctx.save();ctx.translate(x,y);ctx.scale(1,0.5);ctx.fillStyle=P.charShadow;ctx.beginPath();ctx.ellipse(0,0,hw+4,hd+8,0,0,Math.PI*2);ctx.fill();ctx.restore();
  ctx.fillStyle='#6b4f36';
