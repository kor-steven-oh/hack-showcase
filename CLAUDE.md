...
3. **지오메트리** — isometric math: `w2s()` grid→screen projection, `TW/TH` tile size, `HALL_W×HALL_D` (82×43) hall. Booths are grouped per track into `ZONES` (5 zone centers, 2×2 booths each, spacing `BOOTH_DX`/`BOOTH_DY`); track banner + carpet + zone-pill detection all derive from `ZONES`. Central plaza (`PLAZA`) holds the HELLO AI billboard; `PG` is the playground rect (arcade minigame cabinets, decorative for now). `DECOR` lists billboard/trees/benches/arcades (drawn, no collision — only walls collide).
